...
using View.Windows.Forms.Controls.Win32API;
...
namespace View.Windows.Forms.Controls
{
    ...
    public partial class ViewComboBox : UserControl
    {
        ...
        private void DrawImage()
        {
            if (UseAnimation)
            {
                animation.Set(this, Properties.Backcolor, backColor);
                animation.Set(border1, Properties.Backcolor, borderColor);
                animation.Set(border2, Properties.Backcolor, borderColor);
                animation.Set(border3, Properties.Backcolor, borderColor);
                animation.Set(border4, Properties.Backcolor, borderColor);
                animation.Set(lbText, Properties.Forecolor, foreColor);
            }
            else
            {
                BackColor = backColor;
        ...
        private void ViewComboBox_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            {//当列表项目总高度超过最高高度时,为控件添加滚动事件
                //if (UseAnimation)
                //{ animation.Set(panelSelect, Properties.Height, MaxHeight); }
                //else
            ...
            {
                //if (UseAnimation)
                //{ animation.Set(panelSelect, Properties.Height, num); }
                //else
            ...
            {
                if (UseAnimation)
                { animation.Set(panelSelect, Properties.Top, Y); }
            ...
            {//定位到当前选中项
                if (selectHeight > MaxHeight)
                {
                    ...
                    {
                        if (UseAnimation)
                        { animation.Set(labels[j], Properties.Top, labels[j].Top - hs - labels[j].Height - 4); }
        ...
        private void PanelSelect_MouseWheel(object sender, MouseEventArgs e)
        {
            ...
            {
                //if (UseAnimation)
                { animation.Set(labels[j], Properties.Top, labels[j].Top - h); }
            ...
            {//上拉不越界
                ...
                {
                    //if (UseAnimation)
                    { animation.Set(labels[j], Properties.Top, num); }
        ...
        private void Label_MouseLeave(object sender, EventArgs e)
        {
            if (((Label)sender).TabIndex != SelectedIndex)
            {
                ...
                {
                    if (UseAnimation)
                    { animation.Set((Label)sender, Properties.Backcolor, Color.FromArgb(43, 43, 43)); }
                    else { ((Label)sender).BackColor = Color.FromArgb(43, 43, 43); }
                }
                else
                {
                    if (UseAnimation)
                    { animation.Set((Label)sender, Properties.Backcolor, Color.FromArgb(242, 242, 242)); }
                    else
        ...
        {
            if (UseAnimation)
            { animation.Set((Label)sender, Properties.Backcolor, ControlSet.ThemeColor); }
            else
